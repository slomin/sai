...
# without a seal predates #95 and is stable. Checked: the checksums; the
...
if [ -f "$dist/seal" ]; then
  ...
  fail "a $flavor release without a seal; tool/release.sh prepare $flavor writes one"
fi
