...
- A Dart CLI (`sai`) that gathers shell context and forwards requests to the `sai-core` binary.
...
Restart your shell; typing `sai how do I run tests?` now calls the wrapper, which collects recent history + `PWD` and streams a JSON payload to `sai-core`.
