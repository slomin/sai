...
| `default_model` | no | the model used when a request names none. `openrouter` needs it as one exact `owner/name` id: `openrouter/auto`, a `~latest` alias or a `:nitro`/`:floor`/`:online` shortcut leaves the entry misconfigured |
