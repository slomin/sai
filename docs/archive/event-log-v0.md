...
| type | payload |
| --- | --- |
| `provider.request` | `messages` — `[{role, text}]`, `role` ∈ `system` \| `user` \| `assistant`; optional `max_tokens`, `temperature`; `context_hash` — the sha256 blobref of `messages` as sent (ADR 0011), so a response is traceable to exactly what the model saw. The request as sent, so it never holds a key, header or URL. |
