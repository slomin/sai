...
| type | payload |
| --- | --- |
| `provider.request` | `messages` — `[{role, text}]`, `role` ∈ `system` \| `user` \| `assistant`; optional `max_tokens`, `temperature`, `reasoning_effort` (#26 — the effort the caller asked for, the backend's own word: `none` when it asked the model not to think, `high`, `xhigh`, …; absent when the model's default was left to stand; lines written before #26 carry `reasoning: false` for what is now `none`, and stand as written); `context_hash` — the sha256 blobref of `messages` as sent (ADR 0011), so a response is traceable to exactly what the model saw. The request as sent, so it never holds a key, header or URL. |
